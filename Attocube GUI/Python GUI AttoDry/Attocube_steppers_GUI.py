from GUI_lib import (Qt, QEvent, QTimer, label, field, button, buttonWithLed, checkBox, dropDownMenu,box, led, Graphical_interface, traceback)
...
class ANC350controller():
    ...
    def connect(self):
        ...
        self.lib.discover(search_usb=True, search_tcp=False) #Only looks for USB connected devices
...
class ANC350axis():
    ...
    def make_box(self):
        ...
        self.labelContinuous=label("Continuous")
...
class ANC350GUI(Graphical_interface):
    def __init__(self, size='auto',title='sample positionners',stack='sample',stackNumbers=stackNumbers,orientation=sampleOrientation):
        fileFolder=os.path.dirname(os.path.abspath(__file__))
        pathToDll=fileFolder+'\\ANC350_Python_Control-master\\ANC350\\win64\\anc350v4.dll'
        #DLL files found on https://github.com/attocube-systems/ANC350_Python_Control
        self.lib=ANC350v4Lib(path_to_dll=pathToDll)
        self.orientation=orientation
...
if __name__ == "__main__":
    GUI1=ANC350GUI(title='Tip positionners',stack='tip',stackNumbers=stackNumbers,orientation=tipOrientation)
    GUI2=ANC350GUI(title='Sample positionners',stack='sample',stackNumbers=stackNumbers,orientation=sampleOrientation)
    GUI1.run()
